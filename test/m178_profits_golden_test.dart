/// م178 — لقطات للمراجعة (GOLDENS=1 محلياً فقط): قسم الأرباح المعاد
/// بناؤه — الشهرية جداول والسنوية الجديدة (مؤشرات + أرباح وخسائر)
/// وكشف الحساب الجدولي، هاتفاً وكمبيوتراً. عدة الخطوط من م154.
library;
...
void main() {
  ...
  setUp(() => tmp = Directory.systemTemp.createTempSync('m178g_'));
  tearDown(() {
  ...
  testWidgets('لقطة م178 — الشهرية جداول (هاتف)', (t) async {
    await seed();
    await pumpApp(t, desktop: false);
    await openProfitsPhone(t);
    await shot(t, 'm178_profits_monthly_phone');
  }, skip: !_goldens);

  testWidgets('لقطة م178 — السنوية الجديدة (هاتف)', (t) async {
    await seed();
    await pumpApp(t, desktop: false);
    await openProfitsPhone(t);
    await t.tap(find.byKey(const Key('prof-view-yearly')),
        warnIfMissed: false);
    await t.pumpAndSettle();
    await shot(t, 'm178_profits_yearly_phone');
  }, skip: !_goldens);

  testWidgets('لقطة م178 — كشف الحساب الجدولي (هاتف)', (t) async {
    await seed();
    await pumpApp(t, desktop: false);
    await openProfitsPhone(t);
    await t.tap(find.byKey(const Key('prof-view-statement')),
        warnIfMissed: false);
    await t.pumpAndSettle();
    await shot(t, 'm178_profits_statement_phone');
  }, skip: !_goldens);

  testWidgets('لقطة م178 — الشهرية جنباً إلى جنب (كمبيوتر)', (t) async {
    await seed();
    await pumpApp(t, desktop: true);
    await t.tap(find.byKey(const Key('desk-tab-finance')),
        warnIfMissed: false);
    await t.pumpAndSettle();
    await shot(t, 'm178_profits_monthly_desktop');
  }, skip: !_goldens);

  testWidgets('لقطة م178/ب — كشف الحساب الجدولي (كمبيوتر)', (t) async {
    await seed();
    await pumpApp(t, desktop: true);
    await t.tap(find.byKey(const Key('desk-tab-finance')),
        warnIfMissed: false);
    ...
    c.read(desktopProfitsViewProvider.notifier).state = 'statement';
    await t.pumpAndSettle();
    await shot(t, 'm178_profits_statement_desktop');
  }, skip: !_goldens);

  testWidgets('لقطة م178 — السنوية العريضة (كمبيوتر)', (t) async {
    await seed();
    await pumpApp(t, desktop: true);
    await t.tap(find.byKey(const Key('desk-tab-finance')),
        warnIfMissed: false);
    ...
    await t.pumpAndSettle();
    await shot(t, 'm178_profits_yearly_desktop');
  }, skip: !_goldens);
}
