/// م188 — لقطات للمراجعة (GOLDENS=1 محلياً فقط): صفّ «إيراد التحاليل (+)»
/// في جدول الإجمالي العام (هاتفاً وكمبيوتراً) وسطرُه في ذيل السنة —
/// بيانات م178 نفسها مضافاً إليها تحاليلُ الشهر. عدة الخطوط من م154.
library;
...
void main() {
  ...
  setUp(() => tmp = Directory.systemTemp.createTempSync('m188g_'));
  tearDown(() {
  ...
  testWidgets('لقطة م188 — الشهرية بصفّ التحاليل (هاتف)', (t) async {
    await seed();
    await pumpApp(t, desktop: false);
    await openProfitsPhone(t);
    await shot(t, 'm188_profits_monthly_phone');
  }, skip: !_goldens);

  testWidgets('لقطة م188 — الشهرية بصفّ التحاليل (كمبيوتر)', (t) async {
    await seed();
    await pumpApp(t, desktop: true);
    await t.tap(find.byKey(const Key('desk-tab-finance')),
        warnIfMissed: false);
    await t.pumpAndSettle();
    await shot(t, 'm188_profits_monthly_desktop');
  }, skip: !_goldens);

  testWidgets('لقطة م188 — ذيل السنة بسطر التحاليل (هاتف)', (t) async {
    await seed();
    await pumpApp(t, desktop: false);
    await openProfitsPhone(t);
    await t.tap(find.byKey(const Key('prof-view-yearly')),
        warnIfMissed: false);
    await t.pumpAndSettle();
    await shot(t, 'm188_profits_yearly_phone');
  }, skip: !_goldens);
}
